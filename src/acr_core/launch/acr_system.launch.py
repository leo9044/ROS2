...
def generate_launch_description():
    ...
    obstacle = Node(
        ...
        arguments=['-world', 'acr_world', '-name', 'cps_obstacle', '-file',
                   os.path.join(pkg_share, 'models', 'box_obstacle.sdf'),
                   '-x', '1.23', '-y', '0.0', '-z', '0.0'])

    return LaunchDescription([
        DeclareLaunchArgument('spawn_obstacle', default_value='false',
                              description='Spawn the Collision Prevention Stop test obstacle during charging.'),
        DeclareLaunchArgument('obstacle_delay_sec', default_value='5.0',
                              description='Delay in seconds before spawning the Collision Prevention Stop test obstacle.'),
        DeclareLaunchArgument('car_start_delay_sec', default_value='8.0',
                              description='Delay in seconds before CAR_Node starts Service and Action.'),
        DeclareLaunchArgument('control_period_ms', default_value='30',
                              description='ACR trajectory command period in milliseconds.'),
        DeclareLaunchArgument('auth_request_delay_sec', default_value='3.0',
                              description='Demo delay before CAR sends the authentication request.'),
        DeclareLaunchArgument('auth_response_delay_sec', default_value='2.0',
                              description='Demo delay before ACR returns the authentication response.'),
        DeclareLaunchArgument('action_goal_delay_sec', default_value='4.0',
